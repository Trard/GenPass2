...
allow_sm_letter, allow_other = tk.IntVar(), tk.IntVar()
allow_number.set(1)
allow_letter.set(1)
allow_gr_letter.set(1)
allow_sm_letter.set(1)
allow_other.set(1)
select_checksymbolbet_action = tk.IntVar()
...
class CheckUI(UI):
    main_frame = tk.Frame()
    itog_frame = tk.LabelFrame(main_frame, text='Проверка пароля', font='Helvetica 14')
    ...
    def place(self):
        ...
        CheckUI.add_pas_storage_button.pack(side=tk.LEFT, padx=2)
        CheckUI.check_password_button.pack(side=tk.LEFT, padx=2)
        CheckUI.check_password_entry.pack(side=tk.LEFT, padx=2)
